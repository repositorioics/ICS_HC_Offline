package com.ics.ics_hc_offline.dto;

public class HojaConsultaOffLineDTO {
    private int secHojaConsulta;
    private int codExpediente;
    private int numHojaConsulta;
    private int numOrdenLlegada;
    private String estado;
    private String fechaConsulta;
    private int usuarioEnfermeria;
    private int usuarioMedico;
    private Double pesoKg;
    private Double tallaCm;
    private Double temperaturac;
    private String presion;
    private int fciaResp;
    private int fciaCard;
    private String lugarAtencion;
    private String consulta;
    private String segChick;
    private String turno;
    private Double temMedc;
    private String ultDiaFiebre;
    private String ultDosisAntipiretico;
    private String fiebre;
    private String astenia;
    private String asomnoliento;
    private String malEstado;
    private String perdidaConsciencia;
    private String inquieto;
    private String convulsiones;
    private String hipotermia;
    private String letargia;
    private String cefalea;
    private String rigidezCuello;
    private String inyeccionConjuntival;
    private String hemorragiaSuconjuntival;
    private String dolorRetroocular;
    private String fontanelaAbombada;
    private String ictericiaConuntival;
    private String eritema;
    private String dolorGarganta;
    private String adenopatiasCervicales;
    private String exudado;
    private String petequiasMucosa;
    private String tos;
    private String rinorrea;
    private String congestionNasal;
    private String otalgia;
    private String aleteoNasal;
    private String apnea;
    private String respiracionRapida;
    private String quejidoEspiratorio;
    private String estiradorReposo;
    private String tirajeSubcostal;
    private String sibilancias;
    private String crepitos;
    private String roncos;
    private String otraFif;
    private String nuevaFif;
    private String pocoApetito;
    private String nausea;
    private String dificultadAlimentarse;
    private String vomito12horas;
    private String diarrea;
    private String diarreaSangre;
    private String estrenimiento;
    private String dolorAbIntermitente;
    private String dolorAbContinuo;
    private String epigastralgia;
    private String intoleranciaOral;
    private String distensionAbdominal;
    private String hepatomegalia;
    private String lenguaMucosasSecas;
    private String pliegueCutaneo;
    private String orinaReducida;
    private String bebeConSed;
    private String ojosHundidos;
    private String fontanelaHundida;
    private String sintomasUrinarios;
    private String leucocituria;
    private String nitritos;
    private String bilirrubinuria;
    private String altralgia;
    private String mialgia;
    private String lumbalgia;
    private String dolorCuello;
    private String tenosinovitis;
    private String artralgiaProximal;
    private String artralgiaDistal;
    private String conjuntivitis;
    private String edemaMunecas;
    private String edemaCodos;
    private String edemaHombros;
    private String edemaRodillas;
    private String edemaTobillos;
    private String rahsLocalizado;
    private String rahsGeneralizado;
    private String rashEritematoso;
    private String rahsMacular;
    private String rashPapular;
    private String rahsMoteada;
    private String ruborFacial;
    private String equimosis;
    private String cianosisCentral;
    private String ictericia;
    private String obeso;
    private String sobrepeso;
    private String sospechaProblema;
    private String normal;
    private String bajoPeso;
    private String bajoPesoSevero;
    private String lactanciaMaterna;
    private String vacunasCompletas;
    private String vacunaInfluenza;
    private String fechaVacuna;
    private String interconsultaPediatrica;
    private String referenciaHospital;
    private String referenciaDengue;
    private String referenciaIrag;
    private String referenciaChik;
    private String eti;
    private String irag;
    private String neumonia;
    private Integer saturaciono2;
    private Double imc;
    private String categoria;
    private String cambioCategoria;
    private String manifestacionHemorragica;
    private String pruebaTorniquetePositiva;
    private String petequia10Pt;
    private String petequia20Pt;
    private String pielExtremidadesFrias;
    private String palidezEnExtremidades;
    private String epistaxis;
    private String gingivorragia;
    private String petequiasEspontaneas;
    private String llenadoCapilar2seg;
    private String cianosis;
    private Double linfocitosaAtipicos;
    private String fechaLinfocitos;
    private String hipermenorrea;
    private String hematemesis;
    private String melena;
    private Short hemoconcentracion;
    private String hospitalizado;
    private String hospitalizadoEspecificar;
    private String transfusionSangre;
    private String transfusionEspecificar;
    private String tomandoMedicamento;
    private String medicamentoEspecificar;
    private String medicamentoDistinto;
    private String medicamentoDistEspecificar;
    private String bhc;
    private String serologiaDengue;
    private String serologiaChik;
    private String gotaGruesa;
    private String extendidoPeriferico;
    private String ego;
    private String egh;
    private String citologiaFecal;
    private String factorReumatoideo;
    private String albumina;
    private String astAlt;
    private String bilirrubinas;
    private String cpk;
    private String colesterol;
    private String influenza;
    private String otroExamenLab;
    private String oel;
    private Integer numOrdenLaboratorio;
    private String acetaminofen;
    private String asa;
    private String ibuprofen;
    private String penicilina;
    private String amoxicilina;
    private String dicloxacilina;
    private String otroAntibiotico;
    private String furazolidona;
    private String metronidazolTinidazol;
    private String albendazolMebendazol;
    private String sulfatoFerroso;
    private String sueroOral;
    private String sulfatoZinc;
    private String liquidosIv;
    private String prednisona;
    private String hidrocortisonaIv;
    private String salbutamol;
    private String oseltamivir;
    private String historiaExamenFisico;
    private Short diagnostico1;
    private Short diagnostico2;
    private Short diagnostico3;
    private Short diagnostico4;
    private String otroDiagnostico;
    private String proximaCita;
    private String horarioClases;
    private String fechaCierre;
    private String fechaCambioTurno;
    private String fechaCierreCambioTurno;
    private String amPmUltDiaFiebre;
    private String horaUltDosisAntipiretico;
    private String amPmUltDosisAntipiretico;
    private String expedienteFisico;
    private String colegio;
    private String fechaOrdenLaboratorio;
    private String estadoCarga;
    private String otro;
    private String fis;
    private String fif;
    private Double hepatomegaliaCm;
    private String eritrocitos;
    private String planes;
    private int medicoCambioTurno;
    private String hemoconc;
    private Short vomito12h;
    private Short pad;
    private Short pas;
    private Long telef;
    private String hora;
    private String horasv;
    private String noAtiendeLlamadoEnfermeria;
    private String noAtiendeLlamadoMedico;
    private String estudiosParticipantes;
    //private boolean uaf;
    //private String repeatKey; // 07/02/2020 dia en que se agrego
    private String cV; // 05/06/2020
    private String consultaRespiratorio; // 27/11/2020
    //private Short usuarioCierraHoja; // 27/09/2021
    //private String hojaImpresa; // 27/09/2021

    private String esConsultaTerreno;

    public int getSecHojaConsulta() {
        return secHojaConsulta;
    }

    public void setSecHojaConsulta(int secHojaConsulta) {
        this.secHojaConsulta = secHojaConsulta;
    }

    public int getCodExpediente() {
        return codExpediente;
    }

    public void setCodExpediente(int codExpediente) {
        this.codExpediente = codExpediente;
    }

    public String getFechaConsulta() {
        return fechaConsulta;
    }

    public void setFechaConsulta(String fechaConsulta) {
        this.fechaConsulta = fechaConsulta;
    }

    public int getNumHojaConsulta() {
        return numHojaConsulta;
    }

    public void setNumHojaConsulta(int numHojaConsulta) {
        this.numHojaConsulta = numHojaConsulta;
    }

    public Double getPesoKg() {
        return pesoKg;
    }

    public void setPesoKg(Double pesoKg) {
        this.pesoKg = pesoKg;
    }

    public Double getTallaCm() {
        return tallaCm;
    }

    public void setTallaCm(Double tallaCm) {
        this.tallaCm = tallaCm;
    }

    public Double getTemperaturac() {
        return temperaturac;
    }

    public void setTemperaturac(Double temperaturac) {
        this.temperaturac = temperaturac;
    }

    public int getUsuarioEnfermeria() {
        return usuarioEnfermeria;
    }

    public void setUsuarioEnfermeria(int usuarioEnfermeria) {
        this.usuarioEnfermeria = usuarioEnfermeria;
    }

    public String getExpedienteFisico() {
        return expedienteFisico;
    }

    public void setExpedienteFisico(String expedienteFisico) {
        this.expedienteFisico = expedienteFisico;
    }

    public int getNumOrdenLlegada() {
        return numOrdenLlegada;
    }

    public void setNumOrdenLlegada(int numOrdenLlegada) {
        this.numOrdenLlegada = numOrdenLlegada;
    }

    public String getHistoriaExamenFisico() {
        return historiaExamenFisico;
    }

    public void setHistoriaExamenFisico(String historiaExamenFisico) {
        this.historiaExamenFisico = historiaExamenFisico;
    }

    public String getPlanes() {
        return planes;
    }

    public void setPlanes(String planes) {
        this.planes = planes;
    }

    public String getPresion() {
        return presion;
    }

    public void setPresion(String presion) {
        this.presion = presion;
    }

    public Short getPas() {
        return pas;
    }

    public void setPas(Short pas) {
        this.pas = pas;
    }

    public Short getPad() {
        return pad;
    }

    public void setPad(Short pad) {
        this.pad = pad;
    }

    public int getFciaResp() {
        return fciaResp;
    }

    public void setFciaResp(int fciaResp) {
        this.fciaResp = fciaResp;
    }

    public int getFciaCard() {
        return fciaCard;
    }

    public void setFciaCard(int fciaCard) {
        this.fciaCard = fciaCard;
    }

    public String getLugarAtencion() {
        return lugarAtencion;
    }

    public void setLugarAtencion(String lugarAtencion) {
        this.lugarAtencion = lugarAtencion;
    }

    public String getConsulta() {
        return consulta;
    }

    public void setConsulta(String consulta) {
        this.consulta = consulta;
    }

    public String getSegChick() {
        return segChick;
    }

    public void setSegChick(String segChick) {
        this.segChick = segChick;
    }

    public String getTurno() {
        return turno;
    }

    public void setTurno(String turno) {
        this.turno = turno;
    }

    public Double getTemMedc() {
        return temMedc;
    }

    public void setTemMedc(Double temMedc) {
        this.temMedc = temMedc;
    }

    public String getFis() {
        return fis;
    }

    public void setFis(String fis) {
        this.fis = fis;
    }

    public String getFif() {
        return fif;
    }

    public void setFif(String fif) {
        this.fif = fif;
    }

    public String getUltDiaFiebre() {
        return ultDiaFiebre;
    }

    public void setUltDiaFiebre(String ultDiaFiebre) {
        this.ultDiaFiebre = ultDiaFiebre;
    }

    public String getUltDosisAntipiretico() {
        return ultDosisAntipiretico;
    }

    public void setUltDosisAntipiretico(String ultDosisAntipiretico) {
        this.ultDosisAntipiretico = ultDosisAntipiretico;
    }

    public String getAmPmUltDiaFiebre() {
        return amPmUltDiaFiebre;
    }

    public void setAmPmUltDiaFiebre(String amPmUltDiaFiebre) {
        this.amPmUltDiaFiebre = amPmUltDiaFiebre;
    }

    public String getHoraUltDosisAntipiretico() {
        return horaUltDosisAntipiretico;
    }

    public void setHoraUltDosisAntipiretico(String horaUltDosisAntipiretico) {
        this.horaUltDosisAntipiretico = horaUltDosisAntipiretico;
    }

    public String getAmPmUltDosisAntipiretico() {
        return amPmUltDosisAntipiretico;
    }

    public void setAmPmUltDosisAntipiretico(String amPmUltDosisAntipiretico) {
        this.amPmUltDosisAntipiretico = amPmUltDosisAntipiretico;
    }

    public String getFiebre() {
        return fiebre;
    }

    public void setFiebre(String fiebre) {
        this.fiebre = fiebre;
    }

    public String getAstenia() {
        return astenia;
    }

    public void setAstenia(String astenia) {
        this.astenia = astenia;
    }

    public String getAsomnoliento() {
        return asomnoliento;
    }

    public void setAsomnoliento(String asomnoliento) {
        this.asomnoliento = asomnoliento;
    }

    public String getMalEstado() {
        return malEstado;
    }

    public void setMalEstado(String malEstado) {
        this.malEstado = malEstado;
    }

    public String getPerdidaConsciencia() {
        return perdidaConsciencia;
    }

    public void setPerdidaConsciencia(String perdidaConsciencia) {
        this.perdidaConsciencia = perdidaConsciencia;
    }

    public String getInquieto() {
        return inquieto;
    }

    public void setInquieto(String inquieto) {
        this.inquieto = inquieto;
    }

    public String getConvulsiones() {
        return convulsiones;
    }

    public void setConvulsiones(String convulsiones) {
        this.convulsiones = convulsiones;
    }

    public String getHipotermia() {
        return hipotermia;
    }

    public void setHipotermia(String hipotermia) {
        this.hipotermia = hipotermia;
    }

    public String getLetargia() {
        return letargia;
    }

    public void setLetargia(String letargia) {
        this.letargia = letargia;
    }

    public String getPocoApetito() {
        return pocoApetito;
    }

    public void setPocoApetito(String pocoApetito) {
        this.pocoApetito = pocoApetito;
    }

    public String getNausea() {
        return nausea;
    }

    public void setNausea(String nausea) {
        this.nausea = nausea;
    }

    public String getDificultadAlimentarse() {
        return dificultadAlimentarse;
    }

    public void setDificultadAlimentarse(String dificultadAlimentarse) {
        this.dificultadAlimentarse = dificultadAlimentarse;
    }

    public String getVomito12horas() {
        return vomito12horas;
    }

    public void setVomito12horas(String vomito12horas) {
        this.vomito12horas = vomito12horas;
    }

    public Short getVomito12h() {
        return vomito12h;
    }

    public void setVomito12h(Short vomito12h) {
        this.vomito12h = vomito12h;
    }

    public String getDiarrea() {
        return diarrea;
    }

    public void setDiarrea(String diarrea) {
        this.diarrea = diarrea;
    }

    public String getDiarreaSangre() {
        return diarreaSangre;
    }

    public void setDiarreaSangre(String diarreaSangre) {
        this.diarreaSangre = diarreaSangre;
    }

    public String getEstrenimiento() {
        return estrenimiento;
    }

    public void setEstrenimiento(String estrenimiento) {
        this.estrenimiento = estrenimiento;
    }

    public String getDolorAbIntermitente() {
        return dolorAbIntermitente;
    }

    public void setDolorAbIntermitente(String dolorAbIntermitente) {
        this.dolorAbIntermitente = dolorAbIntermitente;
    }

    public String getDolorAbContinuo() {
        return dolorAbContinuo;
    }

    public void setDolorAbContinuo(String dolorAbContinuo) {
        this.dolorAbContinuo = dolorAbContinuo;
    }

    public String getEpigastralgia() {
        return epigastralgia;
    }

    public void setEpigastralgia(String epigastralgia) {
        this.epigastralgia = epigastralgia;
    }

    public String getIntoleranciaOral() {
        return intoleranciaOral;
    }

    public void setIntoleranciaOral(String intoleranciaOral) {
        this.intoleranciaOral = intoleranciaOral;
    }

    public String getDistensionAbdominal() {
        return distensionAbdominal;
    }

    public void setDistensionAbdominal(String distensionAbdominal) {
        this.distensionAbdominal = distensionAbdominal;
    }

    public String getHepatomegalia() {
        return hepatomegalia;
    }

    public void setHepatomegalia(String hepatomegalia) {
        this.hepatomegalia = hepatomegalia;
    }

    public Double getHepatomegaliaCm() {
        return hepatomegaliaCm;
    }

    public void setHepatomegaliaCm(Double hepatomegaliaCm) {
        this.hepatomegaliaCm = hepatomegaliaCm;
    }

    public String getAltralgia() {
        return altralgia;
    }

    public void setAltralgia(String altralgia) {
        this.altralgia = altralgia;
    }

    public String getMialgia() {
        return mialgia;
    }

    public void setMialgia(String mialgia) {
        this.mialgia = mialgia;
    }

    public String getLumbalgia() {
        return lumbalgia;
    }

    public void setLumbalgia(String lumbalgia) {
        this.lumbalgia = lumbalgia;
    }

    public String getDolorCuello() {
        return dolorCuello;
    }

    public void setDolorCuello(String dolorCuello) {
        this.dolorCuello = dolorCuello;
    }

    public String getTenosinovitis() {
        return tenosinovitis;
    }

    public void setTenosinovitis(String tenosinovitis) {
        this.tenosinovitis = tenosinovitis;
    }

    public String getArtralgiaProximal() {
        return artralgiaProximal;
    }

    public void setArtralgiaProximal(String artralgiaProximal) {
        this.artralgiaProximal = artralgiaProximal;
    }

    public String getArtralgiaDistal() {
        return artralgiaDistal;
    }

    public void setArtralgiaDistal(String artralgiaDistal) {
        this.artralgiaDistal = artralgiaDistal;
    }

    public String getConjuntivitis() {
        return conjuntivitis;
    }

    public void setConjuntivitis(String conjuntivitis) {
        this.conjuntivitis = conjuntivitis;
    }

    public String getEdemaMunecas() {
        return edemaMunecas;
    }

    public void setEdemaMunecas(String edemaMunecas) {
        this.edemaMunecas = edemaMunecas;
    }

    public String getEdemaCodos() {
        return edemaCodos;
    }

    public void setEdemaCodos(String edemaCodos) {
        this.edemaCodos = edemaCodos;
    }

    public String getEdemaHombros() {
        return edemaHombros;
    }

    public void setEdemaHombros(String edemaHombros) {
        this.edemaHombros = edemaHombros;
    }

    public String getEdemaRodillas() {
        return edemaRodillas;
    }

    public void setEdemaRodillas(String edemaRodillas) {
        this.edemaRodillas = edemaRodillas;
    }

    public String getEdemaTobillos() {
        return edemaTobillos;
    }

    public void setEdemaTobillos(String edemaTobillos) {
        this.edemaTobillos = edemaTobillos;
    }

    public String getCefalea() {
        return cefalea;
    }

    public void setCefalea(String cefalea) {
        this.cefalea = cefalea;
    }

    public String getRigidezCuello() {
        return rigidezCuello;
    }

    public void setRigidezCuello(String rigidezCuello) {
        this.rigidezCuello = rigidezCuello;
    }

    public String getInyeccionConjuntival() {
        return inyeccionConjuntival;
    }

    public void setInyeccionConjuntival(String inyeccionConjuntival) {
        this.inyeccionConjuntival = inyeccionConjuntival;
    }

    public String getHemorragiaSuconjuntival() {
        return hemorragiaSuconjuntival;
    }

    public void setHemorragiaSuconjuntival(String hemorragiaSuconjuntival) {
        this.hemorragiaSuconjuntival = hemorragiaSuconjuntival;
    }

    public String getDolorRetroocular() {
        return dolorRetroocular;
    }

    public void setDolorRetroocular(String dolorRetroocular) {
        this.dolorRetroocular = dolorRetroocular;
    }

    public String getFontanelaAbombada() {
        return fontanelaAbombada;
    }

    public void setFontanelaAbombada(String fontanelaAbombada) {
        this.fontanelaAbombada = fontanelaAbombada;
    }

    public String getIctericiaConuntival() {
        return ictericiaConuntival;
    }

    public void setIctericiaConuntival(String ictericiaConuntival) {
        this.ictericiaConuntival = ictericiaConuntival;
    }

    public String getLenguaMucosasSecas() {
        return lenguaMucosasSecas;
    }

    public void setLenguaMucosasSecas(String lenguaMucosasSecas) {
        this.lenguaMucosasSecas = lenguaMucosasSecas;
    }

    public String getPliegueCutaneo() {
        return pliegueCutaneo;
    }

    public void setPliegueCutaneo(String pliegueCutaneo) {
        this.pliegueCutaneo = pliegueCutaneo;
    }

    public String getOrinaReducida() {
        return orinaReducida;
    }

    public void setOrinaReducida(String orinaReducida) {
        this.orinaReducida = orinaReducida;
    }

    public String getBebeConSed() {
        return bebeConSed;
    }

    public void setBebeConSed(String bebeConSed) {
        this.bebeConSed = bebeConSed;
    }

    public String getOjosHundidos() {
        return ojosHundidos;
    }

    public void setOjosHundidos(String ojosHundidos) {
        this.ojosHundidos = ojosHundidos;
    }

    public String getFontanelaHundida() {
        return fontanelaHundida;
    }

    public void setFontanelaHundida(String fontanelaHundida) {
        this.fontanelaHundida = fontanelaHundida;
    }

    public String getRahsLocalizado() {
        return rahsLocalizado;
    }

    public void setRahsLocalizado(String rahsLocalizado) {
        this.rahsLocalizado = rahsLocalizado;
    }

    public String getRahsGeneralizado() {
        return rahsGeneralizado;
    }

    public void setRahsGeneralizado(String rahsGeneralizado) {
        this.rahsGeneralizado = rahsGeneralizado;
    }

    public String getRashEritematoso() {
        return rashEritematoso;
    }

    public void setRashEritematoso(String rashEritematoso) {
        this.rashEritematoso = rashEritematoso;
    }

    public String getRahsMacular() {
        return rahsMacular;
    }

    public void setRahsMacular(String rahsMacular) {
        this.rahsMacular = rahsMacular;
    }

    public String getRashPapular() {
        return rashPapular;
    }

    public void setRashPapular(String rashPapular) {
        this.rashPapular = rashPapular;
    }

    public String getRahsMoteada() {
        return rahsMoteada;
    }

    public void setRahsMoteada(String rahsMoteada) {
        this.rahsMoteada = rahsMoteada;
    }

    public String getRuborFacial() {
        return ruborFacial;
    }

    public void setRuborFacial(String ruborFacial) {
        this.ruborFacial = ruborFacial;
    }

    public String getEquimosis() {
        return equimosis;
    }

    public void setEquimosis(String equimosis) {
        this.equimosis = equimosis;
    }

    public String getCianosisCentral() {
        return cianosisCentral;
    }

    public void setCianosisCentral(String cianosisCentral) {
        this.cianosisCentral = cianosisCentral;
    }

    public String getIctericia() {
        return ictericia;
    }

    public void setIctericia(String ictericia) {
        this.ictericia = ictericia;
    }

    public String getEritema() {
        return eritema;
    }

    public void setEritema(String eritema) {
        this.eritema = eritema;
    }

    public String getDolorGarganta() {
        return dolorGarganta;
    }

    public void setDolorGarganta(String dolorGarganta) {
        this.dolorGarganta = dolorGarganta;
    }

    public String getAdenopatiasCervicales() {
        return adenopatiasCervicales;
    }

    public void setAdenopatiasCervicales(String adenopatiasCervicales) {
        this.adenopatiasCervicales = adenopatiasCervicales;
    }

    public String getExudado() {
        return exudado;
    }

    public void setExudado(String exudado) {
        this.exudado = exudado;
    }

    public String getPetequiasMucosa() {
        return petequiasMucosa;
    }

    public void setPetequiasMucosa(String petequiasMucosa) {
        this.petequiasMucosa = petequiasMucosa;
    }

    public String getSintomasUrinarios() {
        return sintomasUrinarios;
    }

    public void setSintomasUrinarios(String sintomasUrinarios) {
        this.sintomasUrinarios = sintomasUrinarios;
    }

    public String getLeucocituria() {
        return leucocituria;
    }

    public void setLeucocituria(String leucocituria) {
        this.leucocituria = leucocituria;
    }

    public String getNitritos() {
        return nitritos;
    }

    public void setNitritos(String nitritos) {
        this.nitritos = nitritos;
    }

    public String getEritrocitos() {
        return eritrocitos;
    }

    public void setEritrocitos(String eritrocitos) {
        this.eritrocitos = eritrocitos;
    }

    public String getBilirrubinuria() {
        return bilirrubinuria;
    }

    public void setBilirrubinuria(String bilirrubinuria) {
        this.bilirrubinuria = bilirrubinuria;
    }

    public String getObeso() {
        return obeso;
    }

    public void setObeso(String obeso) {
        this.obeso = obeso;
    }

    public String getSobrepeso() {
        return sobrepeso;
    }

    public void setSobrepeso(String sobrepeso) {
        this.sobrepeso = sobrepeso;
    }

    public String getSospechaProblema() {
        return sospechaProblema;
    }

    public void setSospechaProblema(String sospechaProblema) {
        this.sospechaProblema = sospechaProblema;
    }

    public String getNormal() {
        return normal;
    }

    public void setNormal(String normal) {
        this.normal = normal;
    }

    public String getBajoPeso() {
        return bajoPeso;
    }

    public void setBajoPeso(String bajoPeso) {
        this.bajoPeso = bajoPeso;
    }

    public String getBajoPesoSevero() {
        return bajoPesoSevero;
    }

    public void setBajoPesoSevero(String bajoPesoSevero) {
        this.bajoPesoSevero = bajoPesoSevero;
    }

    public String getTos() {
        return tos;
    }

    public void setTos(String tos) {
        this.tos = tos;
    }

    public String getRinorrea() {
        return rinorrea;
    }

    public void setRinorrea(String rinorrea) {
        this.rinorrea = rinorrea;
    }

    public String getCongestionNasal() {
        return congestionNasal;
    }

    public void setCongestionNasal(String congestionNasal) {
        this.congestionNasal = congestionNasal;
    }

    public String getOtalgia() {
        return otalgia;
    }

    public void setOtalgia(String otalgia) {
        this.otalgia = otalgia;
    }

    public String getAleteoNasal() {
        return aleteoNasal;
    }

    public void setAleteoNasal(String aleteoNasal) {
        this.aleteoNasal = aleteoNasal;
    }

    public String getApnea() {
        return apnea;
    }

    public void setApnea(String apnea) {
        this.apnea = apnea;
    }

    public String getRespiracionRapida() {
        return respiracionRapida;
    }

    public void setRespiracionRapida(String respiracionRapida) {
        this.respiracionRapida = respiracionRapida;
    }

    public String getQuejidoEspiratorio() {
        return quejidoEspiratorio;
    }

    public void setQuejidoEspiratorio(String quejidoEspiratorio) {
        this.quejidoEspiratorio = quejidoEspiratorio;
    }

    public String getEstiradorReposo() {
        return estiradorReposo;
    }

    public void setEstiradorReposo(String estiradorReposo) {
        this.estiradorReposo = estiradorReposo;
    }

    public String getTirajeSubcostal() {
        return tirajeSubcostal;
    }

    public void setTirajeSubcostal(String tirajeSubcostal) {
        this.tirajeSubcostal = tirajeSubcostal;
    }

    public String getSibilancias() {
        return sibilancias;
    }

    public void setSibilancias(String sibilancias) {
        this.sibilancias = sibilancias;
    }

    public String getCrepitos() {
        return crepitos;
    }

    public void setCrepitos(String crepitos) {
        this.crepitos = crepitos;
    }

    public String getRoncos() {
        return roncos;
    }

    public void setRoncos(String roncos) {
        this.roncos = roncos;
    }

    public String getOtraFif() {
        return otraFif;
    }

    public void setOtraFif(String otraFif) {
        this.otraFif = otraFif;
    }

    public String getNuevaFif() {
        return nuevaFif;
    }

    public void setNuevaFif(String nuevaFif) {
        this.nuevaFif = nuevaFif;
    }

    public String getInterconsultaPediatrica() {
        return interconsultaPediatrica;
    }

    public void setInterconsultaPediatrica(String interconsultaPediatrica) {
        this.interconsultaPediatrica = interconsultaPediatrica;
    }

    public String getReferenciaHospital() {
        return referenciaHospital;
    }

    public void setReferenciaHospital(String referenciaHospital) {
        this.referenciaHospital = referenciaHospital;
    }

    public String getReferenciaDengue() {
        return referenciaDengue;
    }

    public void setReferenciaDengue(String referenciaDengue) {
        this.referenciaDengue = referenciaDengue;
    }

    public String getReferenciaIrag() {
        return referenciaIrag;
    }

    public void setReferenciaIrag(String referenciaIrag) {
        this.referenciaIrag = referenciaIrag;
    }

    public String getReferenciaChik() {
        return referenciaChik;
    }

    public void setReferenciaChik(String referenciaChik) {
        this.referenciaChik = referenciaChik;
    }

    public String getEti() {
        return eti;
    }

    public void setEti(String eti) {
        this.eti = eti;
    }

    public String getIrag() {
        return irag;
    }

    public void setIrag(String irag) {
        this.irag = irag;
    }

    public String getNeumonia() {
        return neumonia;
    }

    public void setNeumonia(String neumonia) {
        this.neumonia = neumonia;
    }

    public String getLactanciaMaterna() {
        return lactanciaMaterna;
    }

    public void setLactanciaMaterna(String lactanciaMaterna) {
        this.lactanciaMaterna = lactanciaMaterna;
    }

    public String getVacunasCompletas() {
        return vacunasCompletas;
    }

    public void setVacunasCompletas(String vacunasCompletas) {
        this.vacunasCompletas = vacunasCompletas;
    }

    public String getVacunaInfluenza() {
        return vacunaInfluenza;
    }

    public void setVacunaInfluenza(String vacunaInfluenza) {
        this.vacunaInfluenza = vacunaInfluenza;
    }

    public String getFechaVacuna() {
        return fechaVacuna;
    }

    public void setFechaVacuna(String fechaVacuna) {
        this.fechaVacuna = fechaVacuna;
    }

    public Integer getSaturaciono2() {
        return saturaciono2;
    }

    public void setSaturaciono2(Integer saturaciono2) {
        this.saturaciono2 = saturaciono2;
    }

    public Double getImc() {
        return imc;
    }

    public void setImc(Double imc) {
        this.imc = imc;
    }

    public String getCategoria() {
        return categoria;
    }

    public void setCategoria(String categoria) {
        this.categoria = categoria;
    }

    public String getCambioCategoria() {
        return cambioCategoria;
    }

    public void setCambioCategoria(String cambioCategoria) {
        this.cambioCategoria = cambioCategoria;
    }

    public String getManifestacionHemorragica() {
        return manifestacionHemorragica;
    }

    public void setManifestacionHemorragica(String manifestacionHemorragica) {
        this.manifestacionHemorragica = manifestacionHemorragica;
    }

    public String getPruebaTorniquetePositiva() {
        return pruebaTorniquetePositiva;
    }

    public void setPruebaTorniquetePositiva(String pruebaTorniquetePositiva) {
        this.pruebaTorniquetePositiva = pruebaTorniquetePositiva;
    }

    public String getPetequia10Pt() {
        return petequia10Pt;
    }

    public void setPetequia10Pt(String petequia10Pt) {
        this.petequia10Pt = petequia10Pt;
    }

    public String getPetequia20Pt() {
        return petequia20Pt;
    }

    public void setPetequia20Pt(String petequia20Pt) {
        this.petequia20Pt = petequia20Pt;
    }

    public String getPielExtremidadesFrias() {
        return pielExtremidadesFrias;
    }

    public void setPielExtremidadesFrias(String pielExtremidadesFrias) {
        this.pielExtremidadesFrias = pielExtremidadesFrias;
    }

    public String getPalidezEnExtremidades() {
        return palidezEnExtremidades;
    }

    public void setPalidezEnExtremidades(String palidezEnExtremidades) {
        this.palidezEnExtremidades = palidezEnExtremidades;
    }

    public String getEpistaxis() {
        return epistaxis;
    }

    public void setEpistaxis(String epistaxis) {
        this.epistaxis = epistaxis;
    }

    public String getGingivorragia() {
        return gingivorragia;
    }

    public void setGingivorragia(String gingivorragia) {
        this.gingivorragia = gingivorragia;
    }

    public String getPetequiasEspontaneas() {
        return petequiasEspontaneas;
    }

    public void setPetequiasEspontaneas(String petequiasEspontaneas) {
        this.petequiasEspontaneas = petequiasEspontaneas;
    }

    public String getLlenadoCapilar2seg() {
        return llenadoCapilar2seg;
    }

    public void setLlenadoCapilar2seg(String llenadoCapilar2seg) {
        this.llenadoCapilar2seg = llenadoCapilar2seg;
    }

    public String getCianosis() {
        return cianosis;
    }

    public void setCianosis(String cianosis) {
        this.cianosis = cianosis;
    }

    public Double getLinfocitosaAtipicos() {
        return linfocitosaAtipicos;
    }

    public void setLinfocitosaAtipicos(Double linfocitosaAtipicos) {
        this.linfocitosaAtipicos = linfocitosaAtipicos;
    }

    public String getFechaLinfocitos() {
        return fechaLinfocitos;
    }

    public void setFechaLinfocitos(String fechaLinfocitos) {
        this.fechaLinfocitos = fechaLinfocitos;
    }

    public String getHipermenorrea() {
        return hipermenorrea;
    }

    public void setHipermenorrea(String hipermenorrea) {
        this.hipermenorrea = hipermenorrea;
    }

    public String getHematemesis() {
        return hematemesis;
    }

    public void setHematemesis(String hematemesis) {
        this.hematemesis = hematemesis;
    }

    public String getMelena() {
        return melena;
    }

    public void setMelena(String melena) {
        this.melena = melena;
    }

    public String getHemoconc() {
        return hemoconc;
    }

    public void setHemoconc(String hemoconc) {
        this.hemoconc = hemoconc;
    }

    public Short getHemoconcentracion() {
        return hemoconcentracion;
    }

    public void setHemoconcentracion(Short hemoconcentracion) {
        this.hemoconcentracion = hemoconcentracion;
    }

    public String getHospitalizado() {
        return hospitalizado;
    }

    public void setHospitalizado(String hospitalizado) {
        this.hospitalizado = hospitalizado;
    }

    public String getHospitalizadoEspecificar() {
        return hospitalizadoEspecificar;
    }

    public void setHospitalizadoEspecificar(String hospitalizadoEspecificar) {
        this.hospitalizadoEspecificar = hospitalizadoEspecificar;
    }

    public String getTransfusionSangre() {
        return transfusionSangre;
    }

    public void setTransfusionSangre(String transfusionSangre) {
        this.transfusionSangre = transfusionSangre;
    }

    public String getTransfusionEspecificar() {
        return transfusionEspecificar;
    }

    public void setTransfusionEspecificar(String transfusionEspecificar) {
        this.transfusionEspecificar = transfusionEspecificar;
    }

    public String getTomandoMedicamento() {
        return tomandoMedicamento;
    }

    public void setTomandoMedicamento(String tomandoMedicamento) {
        this.tomandoMedicamento = tomandoMedicamento;
    }

    public String getMedicamentoEspecificar() {
        return medicamentoEspecificar;
    }

    public void setMedicamentoEspecificar(String medicamentoEspecificar) {
        this.medicamentoEspecificar = medicamentoEspecificar;
    }

    public String getMedicamentoDistinto() {
        return medicamentoDistinto;
    }

    public void setMedicamentoDistinto(String medicamentoDistinto) {
        this.medicamentoDistinto = medicamentoDistinto;
    }

    public String getMedicamentoDistEspecificar() {
        return medicamentoDistEspecificar;
    }

    public void setMedicamentoDistEspecificar(String medicamentoDistEspecificar) {
        this.medicamentoDistEspecificar = medicamentoDistEspecificar;
    }

    public String getHorasv() {
        return horasv;
    }

    public void setHorasv(String horasv) {
        this.horasv = horasv;
    }

    public String getAsa() {
        return asa;
    }

    public void setAsa(String asa) {
        this.asa = asa;
    }

    public String getIbuprofen() {
        return ibuprofen;
    }

    public void setIbuprofen(String ibuprofen) {
        this.ibuprofen = ibuprofen;
    }

    public String getPenicilina() {
        return penicilina;
    }

    public void setPenicilina(String penicilina) {
        this.penicilina = penicilina;
    }

    public String getAmoxicilina() {
        return amoxicilina;
    }

    public void setAmoxicilina(String amoxicilina) {
        this.amoxicilina = amoxicilina;
    }

    public String getDicloxacilina() {
        return dicloxacilina;
    }

    public void setDicloxacilina(String dicloxacilina) {
        this.dicloxacilina = dicloxacilina;
    }

    public String getOtroAntibiotico() {
        return otroAntibiotico;
    }

    public void setOtroAntibiotico(String otroAntibiotico) {
        this.otroAntibiotico = otroAntibiotico;
    }

    public String getFurazolidona() {
        return furazolidona;
    }

    public void setFurazolidona(String furazolidona) {
        this.furazolidona = furazolidona;
    }

    public String getMetronidazolTinidazol() {
        return metronidazolTinidazol;
    }

    public void setMetronidazolTinidazol(String metronidazolTinidazol) {
        this.metronidazolTinidazol = metronidazolTinidazol;
    }

    public String getAlbendazolMebendazol() {
        return albendazolMebendazol;
    }

    public void setAlbendazolMebendazol(String albendazolMebendazol) {
        this.albendazolMebendazol = albendazolMebendazol;
    }

    public String getSulfatoFerroso() {
        return sulfatoFerroso;
    }

    public void setSulfatoFerroso(String sulfatoFerroso) {
        this.sulfatoFerroso = sulfatoFerroso;
    }

    public String getSueroOral() {
        return sueroOral;
    }

    public void setSueroOral(String sueroOral) {
        this.sueroOral = sueroOral;
    }

    public String getSulfatoZinc() {
        return sulfatoZinc;
    }

    public void setSulfatoZinc(String sulfatoZinc) {
        this.sulfatoZinc = sulfatoZinc;
    }

    public String getLiquidosIv() {
        return liquidosIv;
    }

    public void setLiquidosIv(String liquidosIv) {
        this.liquidosIv = liquidosIv;
    }

    public String getPrednisona() {
        return prednisona;
    }

    public void setPrednisona(String prednisona) {
        this.prednisona = prednisona;
    }

    public String getHidrocortisonaIv() {
        return hidrocortisonaIv;
    }

    public void setHidrocortisonaIv(String hidrocortisonaIv) {
        this.hidrocortisonaIv = hidrocortisonaIv;
    }

    public String getSalbutamol() {
        return salbutamol;
    }

    public void setSalbutamol(String salbutamol) {
        this.salbutamol = salbutamol;
    }

    public String getOseltamivir() {
        return oseltamivir;
    }

    public void setOseltamivir(String oseltamivir) {
        this.oseltamivir = oseltamivir;
    }

    public Long getTelef() {
        return telef;
    }

    public void setTelef(Long telef) {
        this.telef = telef;
    }

    public String getProximaCita() {
        return proximaCita;
    }

    public void setProximaCita(String proximaCita) {
        this.proximaCita = proximaCita;
    }

    public String getHorarioClases() {
        return horarioClases;
    }

    public void setHorarioClases(String horarioClases) {
        this.horarioClases = horarioClases;
    }

    public String getColegio() {
        return colegio;
    }

    public void setColegio(String colegio) {
        this.colegio = colegio;
    }

    public String getBhc() {
        return bhc;
    }

    public void setBhc(String bhc) {
        this.bhc = bhc;
    }

    public String getSerologiaDengue() {
        return serologiaDengue;
    }

    public void setSerologiaDengue(String serologiaDengue) {
        this.serologiaDengue = serologiaDengue;
    }

    public String getSerologiaChik() {
        return serologiaChik;
    }

    public void setSerologiaChik(String serologiaChik) {
        this.serologiaChik = serologiaChik;
    }

    public String getGotaGruesa() {
        return gotaGruesa;
    }

    public void setGotaGruesa(String gotaGruesa) {
        this.gotaGruesa = gotaGruesa;
    }

    public String getExtendidoPeriferico() {
        return extendidoPeriferico;
    }

    public void setExtendidoPeriferico(String extendidoPeriferico) {
        this.extendidoPeriferico = extendidoPeriferico;
    }

    public String getEgo() {
        return ego;
    }

    public void setEgo(String ego) {
        this.ego = ego;
    }

    public String getEgh() {
        return egh;
    }

    public void setEgh(String egh) {
        this.egh = egh;
    }

    public String getCitologiaFecal() {
        return citologiaFecal;
    }

    public void setCitologiaFecal(String citologiaFecal) {
        this.citologiaFecal = citologiaFecal;
    }

    public String getFactorReumatoideo() {
        return factorReumatoideo;
    }

    public void setFactorReumatoideo(String factorReumatoideo) {
        this.factorReumatoideo = factorReumatoideo;
    }

    public String getAlbumina() {
        return albumina;
    }

    public void setAlbumina(String albumina) {
        this.albumina = albumina;
    }

    public String getAstAlt() {
        return astAlt;
    }

    public void setAstAlt(String astAlt) {
        this.astAlt = astAlt;
    }

    public String getBilirrubinas() {
        return bilirrubinas;
    }

    public void setBilirrubinas(String bilirrubinas) {
        this.bilirrubinas = bilirrubinas;
    }

    public String getCpk() {
        return cpk;
    }

    public void setCpk(String cpk) {
        this.cpk = cpk;
    }

    public String getColesterol() {
        return colesterol;
    }

    public void setColesterol(String colesterol) {
        this.colesterol = colesterol;
    }

    public String getInfluenza() {
        return influenza;
    }

    public void setInfluenza(String influenza) {
        this.influenza = influenza;
    }

    public String getOtroExamenLab() {
        return otroExamenLab;
    }

    public void setOtroExamenLab(String otroExamenLab) {
        this.otroExamenLab = otroExamenLab;
    }

    public String getOel() {
        return oel;
    }

    public void setOel(String oel) {
        this.oel = oel;
    }

    public Integer getNumOrdenLaboratorio() {
        return numOrdenLaboratorio;
    }

    public void setNumOrdenLaboratorio(Integer numOrdenLaboratorio) {
        this.numOrdenLaboratorio = numOrdenLaboratorio;
    }

    public String getFechaOrdenLaboratorio() {
        return fechaOrdenLaboratorio;
    }

    public void setFechaOrdenLaboratorio(String fechaOrdenLaboratorio) {
        this.fechaOrdenLaboratorio = fechaOrdenLaboratorio;
    }

    public short getDiagnostico1() {
        return diagnostico1;
    }

    public void setDiagnostico1(short diagnostico1) {
        this.diagnostico1 = diagnostico1;
    }

    public short getDiagnostico2() {
        return diagnostico2;
    }

    public void setDiagnostico2(short diagnostico2) {
        this.diagnostico2 = diagnostico2;
    }

    public short getDiagnostico3() {
        return diagnostico3;
    }

    public void setDiagnostico3(short diagnostico3) {
        this.diagnostico3 = diagnostico3;
    }

    public short getDiagnostico4() {
        return diagnostico4;
    }

    public void setDiagnostico4(short diagnostico4) {
        this.diagnostico4 = diagnostico4;
    }

    public String getOtroDiagnostico() {
        return otroDiagnostico;
    }

    public void setOtroDiagnostico(String otroDiagnostico) {
        this.otroDiagnostico = otroDiagnostico;
    }

    public String getFechaCierre() {
        return fechaCierre;
    }

    public void setFechaCierre(String fechaCierre) {
        this.fechaCierre = fechaCierre;
    }

    public int getUsuarioMedico() {
        return usuarioMedico;
    }

    public void setUsuarioMedico(int usuarioMedico) {
        this.usuarioMedico = usuarioMedico;
    }

    public int getMedicoCambioTurno() {
        return medicoCambioTurno;
    }

    public void setMedicoCambioTurno(int medicoCambioTurno) {
        this.medicoCambioTurno = medicoCambioTurno;
    }

    public String getEstado() {
        return estado;
    }

    public void setEstado(String estado) {
        this.estado = estado;
    }

    public String getOtro() {
        return otro;
    }

    public void setOtro(String otro) {
        this.otro = otro;
    }

    public String getEstudiosParticipantes() {
        return estudiosParticipantes;
    }

    public void setEstudiosParticipantes(String estudiosParticipantes) {
        this.estudiosParticipantes = estudiosParticipantes;
    }

    public String getcV() {
        return cV;
    }

    public void setcV(String cV) {
        this.cV = cV;
    }

    public String getConsultaRespiratorio() {
        return consultaRespiratorio;
    }

    public void setConsultaRespiratorio(String consultaRespiratorio) {
        this.consultaRespiratorio = consultaRespiratorio;
    }

    public String getEsConsultaTerreno() {
        return esConsultaTerreno;
    }

    public void setEsConsultaTerreno(String esConsultaTerreno) {
        this.esConsultaTerreno = esConsultaTerreno;
    }

    public String getAcetaminofen() {
        return acetaminofen;
    }

    public void setAcetaminofen(String acetaminofen) {
        this.acetaminofen = acetaminofen;
    }

    public String getFechaCambioTurno() {
        return fechaCambioTurno;
    }

    public void setFechaCambioTurno(String fechaCambioTurno) {
        this.fechaCambioTurno = fechaCambioTurno;
    }

    public String getFechaCierreCambioTurno() {
        return fechaCierreCambioTurno;
    }

    public void setFechaCierreCambioTurno(String fechaCierreCambioTurno) {
        this.fechaCierreCambioTurno = fechaCierreCambioTurno;
    }

    public String getEstadoCarga() {
        return estadoCarga;
    }

    public void setEstadoCarga(String estadoCarga) {
        this.estadoCarga = estadoCarga;
    }

    public String getHora() {
        return hora;
    }

    public void setHora(String hora) {
        this.hora = hora;
    }

    public String getNoAtiendeLlamadoEnfermeria() {
        return noAtiendeLlamadoEnfermeria;
    }

    public void setNoAtiendeLlamadoEnfermeria(String noAtiendeLlamadoEnfermeria) {
        this.noAtiendeLlamadoEnfermeria = noAtiendeLlamadoEnfermeria;
    }

    public String getNoAtiendeLlamadoMedico() {
        return noAtiendeLlamadoMedico;
    }

    public void setNoAtiendeLlamadoMedico(String noAtiendeLlamadoMedico) {
        this.noAtiendeLlamadoMedico = noAtiendeLlamadoMedico;
    }
}
